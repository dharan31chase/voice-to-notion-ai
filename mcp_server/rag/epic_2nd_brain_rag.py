"""
Epic 2nd Brain RAG Implementation (Phase 6.4)

Privacy-first RAG for Epic 2nd Brain infrastructure documents.
Separate ChromaDB collection from Legacy AI.
"""
...
class Epic2ndBrainRAG(BaseRAG):
    """
    RAG implementation for Epic 2nd Brain project.

    Indexes:
    - PRDs (docs/prd/)
    - Tech requirements (docs/tech-requirements/)
    - Claude Code sessions (docs/sessions/claude-code/)
    - Claude Chat sessions (docs/sessions/claude-chat/)
    - One-pagers (docs/context/one-pagers/)

    Privacy: Isolated ChromaDB collection, no cross-project data.
    """

    def __init__(self, config_path: Optional[Path] = None):
        """Initialize Epic 2nd Brain RAG."""
        super().__init__(project_name="Epic 2nd Brain", config_path=config_path)

        # Lazy-loaded components
        self._initialized = False

    def get_repo_path(self) -> Path:
        """Get Epic 2nd Brain repository path."""
        return Path.home() / "Documents/1. Projects/ai-assistant"
    ...
    def _classify_doc_type(self, file_path: Path) -> str:
        ...
        if "docs/prd" in path_str:
            return "prd"
        elif "docs/tech-requirements" in path_str:
            return "tech-req"
        elif "docs/sessions/claude-code" in path_str:
            return "session-code"
        elif "docs/sessions/claude-chat" in path_str:
            return "session-chat"
        elif "docs/context/one-pagers" in path_str:
            return "one-pager"
        else:
            return "other"
    ...
    def index_documents(self, force_reindex: bool = False) -> Dict[str, Any]:
        """
        Index all Epic 2nd Brain documents.
    ...
    def search(
        ...
        """
        Search Epic 2nd Brain documents.

        Args:
            query: Search query
            top_k: Number of results (default: max_results)
            filter_metadata: Optional metadata filters (e.g., {"doc_type": "prd"})
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get RAG statistics for Epic 2nd Brain."""
        self._ensure_initialized()
